...
conflictdata["columns"].forEach((column, columnI, columnA) => {
    ...
    conflictdata["conflicts"].forEach((currentValue, index, arr) => {
        ...
        <input id="conflict_${index}_side1" type="radio" name="conflict_${index}" value="${currentValue["side1"]}"><img id="conflict_${index}_side1btn" onclick="buttonClick(1, ${index})" class="flag" src="${countrydata["flags"][currentValue["side1"]]}" />
        <input id="conflict_${index}_na" type="radio" name="conflict_${index}" value="neither">
        <img onclick="buttonClick(2, ${index})" id="conflict_${index}_side2btn" class="flag" src="${countrydata["flags"][currentValue["side2"]]}" /><input id="conflict_${index}_side2" type="radio" name="conflict_${index}" value="${currentValue["side2"]}">
        `)
